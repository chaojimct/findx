...
extern "C" __declspec(dllexport) bool fx2_start_indexing(
...
extern "C" __declspec(dllexport) bool fx2_is_indexing() {
  return g_is_indexing.load(std::memory_order_acquire);
}

extern "C" __declspec(dllexport) bool fx2_is_index_ready() {
  return g_is_ready.load(std::memory_order_acquire);
}

extern "C" __declspec(dllexport) uint64_t fx2_indexed_file_count() {
  return g_indexed_count.load(std::memory_order_acquire);
}

extern "C" __declspec(dllexport) const char* fx2_last_error() {
  thread_local std::string error_cache;
  error_cache = ReadLastErrorText();
  return error_cache.c_str();
}

extern "C" __declspec(dllexport) char* fx2_list_drives_json() {
  const std::vector<DriveInfo> rows = list_drives_internal();
...
extern "C" __declspec(dllexport) char* fx2_search_files_json(
...
extern "C" __declspec(dllexport) bool fx2_cancel_search() {
  g_search_request_token.fetch_add(1, std::memory_order_acq_rel);
  return true;
}

extern "C" __declspec(dllexport) char* fx2_find_duplicates_json(
...
extern "C" __declspec(dllexport) bool fx2_cancel_duplicate_scan() {
...
extern "C" __declspec(dllexport) char* fx2_duplicate_scan_status_json() {
...
extern "C" __declspec(dllexport) bool fx2_delete_path(const char* path_utf8,
                                                       bool recycle_bin) {
...
extern "C" __declspec(dllexport) void fx2_free_string(char* value) {
  if (value != nullptr) {
    std::free(value);
  }
}
