...
- **墓碑自动压缩（无感回收）**：此前墓碑比超阈值（≥ 10 万条目且 > 25%）只在设置页提示，需要用户手动点「压缩索引」。现在 service 在**启动窗口内自动完成**：加载索引后、引擎挂上管道前这个零并发空档，体检命中即执行物理压缩（`compact_tombstones` 十处下标重映射）+ 灾难性损失守卫 + 原子落盘（`.compact.tmp` → rename），压缩期间 Status 新增「自动压缩墓碑」阶段（9/9，GUI 状态栏可见进度与目标墓碑数），随后 trigram 边车走既有后台补建路径重建并热替换，期间搜索全表扫描兜底。墓碑主要来自增量删除的日积月累（实测一天 2–4%），每次开机启动体检一次，占比永远到不了高水位——用户全程无感。GUI 手动「一键压缩」保留作兜底。落盘失败（磁盘满等）只降级提示（内存已回收，下次启动重试）；守卫触发则拒绝以可疑索引启动，与 CLI `compact` 同一逻辑。Windows / Unix 两侧启动编排同步接入。
