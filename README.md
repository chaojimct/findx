...
**自动发版**：对 `v*` 标签（例如 `v2.0.4`）推送会触发 [`.github/workflows/release.yml`](.github/workflows/release.yml)，在 `windows-latest` 上 `tauri build --no-bundle` 产出程序与 `resources` 后，用 **[Inno Setup 6](https://jrsoftware.org/isinfo.php)** 执行 [`installer/FindX.iss`](installer/FindX.iss) 打 **`FindX-<ver>-setup.exe`**，并（在 tag 场景下）上传到 [Release](https://github.com/chaojimct/findx/releases)。
...
- **Inno** 安装包版本在 CI 中由 **`/DMyAppVersion=`** 传入 [`installer/FindX.iss`](installer/FindX.iss)（与 tag 如 `v2.0.4` 的纯数字部分一致即可）；`iss` 内 `#define MyAppVersion` 为本地无参数编译时的默认。
